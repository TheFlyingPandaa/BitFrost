...
float4 main(VS_OUT input) : SV_Target
{
    ...
    float3 diffuse = difTex * max(dot(normal, posToLight), 0.2f); //calculate the diffuse factor
		
		// final specular
    float3 specular = spec;
    //spec *= -1;
    return float4(ambb + specular + diffuse, 1.0);
}